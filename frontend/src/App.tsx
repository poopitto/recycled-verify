      ...
      <div className="layout">
        ...
        <div>
          ...
          <RegisterPanel
            ...
            onSubmit={(form, bondWei) =>
              run("register_lot", async () => {
                const id = await registerLot(walletClient, { ...form, bondWei });
                setSelectedId(id);
              })
            }
          />
          ...
          <AdminPanel
            ...
            onAdvance={() => run("advance_epoch", () => advanceEpoch(walletClient))}
            onSetAdmin={(a) => run("set_admin", () => setAdmin(walletClient, a))}
          />
        </div>
      ...
      <div className="panel">
        ...
        <div className="body">
          ...
          <dl className="kv">
            ...
            <dt>Evidence</dt>
            <dd>
              {lot.evidenceVerified
                ? `verified by ${lot.evidenceSourcesConfirmed} source(s)`
                : lot.evidenceDigest
                  ? `pending / not verified (${lot.evidenceDigest})`
                  : "not attached"}
            </dd>
          </dl>
          {lot.verificationSummary && (
            <>
              <div className="label" style={{ margin: "10px 0 4px" }}>External evidence summary</div>
              <div className="rationale">{lot.verificationSummary}</div>
            </>
          )}
          {lot.rationale && (
            <>
          ...
          {can.verify && (
            <div className="body" style={{ borderTop: "2px dashed #161616" }}>
              ...
              <button
                ...
                onClick={() => onAction("verify_mass_balance", () => verifyMassBalance(walletClient, lot.lotId))}
              >
        ...
        <div className="body">
          {can.trace && (
            <div className="field">
              ...
              />
              <label style={{ marginTop: 8 }}>Certifier / lab evidence URLs</label>
              <textarea
                value={evidenceUrls}
                onChange={(e) => setEvidenceUrls(e.target.value)}
                placeholder={"Two or three independent public HTTPS URLs\none certifier/lab record per line"}
              />
              <button
                className="btn hivis"
                disabled={
                  busy === "submit_trace" ||
                  trace.trim().length < 30 ||
                  evidenceUrls
                    .split(/\r?\n/)
                    .filter((url) => url.trim().startsWith("https://")).length < 2
                }
                onClick={() => onAction("submit_trace", () => submitTrace(walletClient, lot.lotId, trace, evidenceUrls))}
                style={{ marginTop: 6 }}
              >
                {busy === "submit_trace" ? <span className="spin" /> : null} Submit trace + external evidence
              </button>
            </div>
          )}
          ...
          <div className="btn-row">
            {can.t1 && (
              <button
                ...
                onClick={() => onAction("adjudicate", () => adjudicate(walletClient, lot.lotId))}
              >
            ...
            {lot.status === ST_AUDITED_T1 && (
              <button
                ...
                onClick={() => onAction("adjudicate_deep", () => adjudicateDeep(walletClient, lot.lotId))}
              >
            ...
            {can.label && (
              <button
                ...
                onClick={() => onAction("issue_label", () => issueLabel(walletClient, lot.lotId))}
              >
            ...
            {can.cascade && (
              <button
                ...
                onClick={() =>
                  onAction("cascade_flag_descendants", () => cascadeFlagDescendants(walletClient, lot.lotId))
                }
              >
